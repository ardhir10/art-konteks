...
class PertekPekerjaanBawahAirController extends Controller
{
    public function show(Request $request, $id)
    {
        $data['page_title'] = 'KEGIATAN PEKERJAAN BAWAH AIR';
        $data['data'] = PermohonanPTPekerjaanBawahAir::findOrFail($id);
        return view('permohonan.pertimbangan-teknis.pekerjaan-bawah-air.show', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('dokp_file_dokumen')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/pba/dokp/');
            ...
            if ($request->hasFile('peta_laut')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/pba/pl/');
                $file->move($destinationPath, $name);
            ...
            if ($request->hasFile('surat_permohonan')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/pba/sp/');
                $file->move($destinationPath, $name);
                $dataPermohonan['surat_permohonan'] = $name;
            }

            $insert = PermohonanPTPekerjaanBawahAir::create($dataPermohonan);
            ...
            foreach ($request->lp_long_degrees as $key => $koordinat) {
                ...
                TitikKordinat::create([
                    ...
                    'table' => 'permohonan_pt_pba',
                    'type' => 'lokasi_pekerjaan_bawah_air',
            ...
            DB::commit();
            return redirect()->route('permohonan.pertimbangan-teknis.pekerjaan-bawah-air.show', ['id' => $insert->id])->with(['success' => 'Data berhasil dibuat !']);
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    {
        $type = 'Pertek/PekerjaanBawahAir';
        $bulanRomawi = $this->getRomawi(date('m'));
        $year_now = date('Y');

        $obj = DB::table('permohonan_pt_pba')
