...
class PertekReklamasiController extends Controller
{
    public function show(Request $request, $id)
    {
        $data['page_title'] = 'KEGIATAN REKLAMASI';
        $data['data'] = PermohonanPTReklamasi::findOrFail($id);
        return view('permohonan.pertimbangan-teknis.reklamasi.show', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $messages = [
            ...
            'dokper_nama_instansi.required' => 'This field is required !',
            'dokper_tanggal_dokumen.required' => 'This field is required !',
            'dokper_berlaku_hingga.required' => 'This field is required !',
            'dokper_file_dokumen.required' => 'This field is required !',
        ];
        $request->validate([
            'perihal' => 'required',
            'lokasi_reklamasi' => 'required',
            ...
            'dokper_nama_instansi' => 'required',
            'dokper_tanggal_dokumen' => 'required',
            'dokper_berlaku_hingga' => 'required',
            'dokper_file_dokumen' => 'required|mimes:pdf,jpg,jpeg,png',
        ...
        $dataPermohonan['lokasi_reklamasi'] = $request->lokasi_reklamasi;
        ...
        $dataPermohonan['dokper_nama_instansi'] = $request->dokper_nama_instansi;
        $dataPermohonan['dokper_tanggal_dokumen'] = $request->dokper_tanggal_dokumen;
        $dataPermohonan['dokper_berlaku_hingga'] = $request->dokper_berlaku_hingga;
        $dataPermohonan['dokper_berlaku_hingga_tanggal'] = $request->dokper_berlaku_hingga_tanggal;
        $dataPermohonan['dokper_file_dokumen'] = null;
        ...
        try {
            ...
            if ($request->hasFile('amdal_file_dokumen')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/reklamasi/amdal/');
            ...
            if ($request->hasFile('dokp_file_dokumen')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/reklamasi/dokp/');
                $file->move($destinationPath, $name);
                $dataPermohonan['dokp_file_dokumen'] = $name;
            }

            // --- UPLOAD DOKUMEN PERTIMBANGAN
            if ($request->hasFile('dokper_file_dokumen')) {
                $file = $request->file('dokper_file_dokumen');
                $name = Auth::user()->id . '_' . 'DOKPER_' . time() . '.' . $file->getClientOriginalExtension();
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/reklamasi/dokper/');
                $file->move($destinationPath, $name);
                $dataPermohonan['dokper_file_dokumen'] = $name;
            }
            ...
            if ($request->hasFile('peta_laut')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/reklamasi/pl/');
                $file->move($destinationPath, $name);
            ...
            if ($request->hasFile('surat_permohonan')) {
                ...
                $destinationPath = public_path('dokumen-permohonan/permohonan-teknis/reklamasi/sp/');
                $file->move($destinationPath, $name);
                $dataPermohonan['surat_permohonan'] = $name;
            }

            $insert = PermohonanPTReklamasi::create($dataPermohonan);

            // --- KORDINAT TITIK REKLAMASI
            $orderTd = 1;
            foreach ($request->kr_long_degrees as $key => $koordinat) {
                $longDecimal = $request->kr_long_degrees[$key] . "° " . $request->kr_long_minutes[$key] . "' " . $request->kr_long_second[$key] . "\" " . $request->kr_long_direction[$key];
                $latDecimal = $request->kr_lat_degrees[$key] . "° " . $request->kr_lat_minutes[$key] . "' " . $request->kr_lat_second[$key] . "\" " . $request->kr_lat_direction[$key];
                ...
                TitikKordinat::create([
                    ...
                    'table' => 'permohonan_pt_reklamasi',
                    'type' => 'lokasi_reklamasi',
                    'long_degrees' => $request->kr_long_degrees[$key],
                    'long_minutes' => $request->kr_long_minutes[$key],
                    'long_second' => $request->kr_long_second[$key],
                    'long_direction' => $request->kr_long_direction[$key],
                    'lat_degrees' => $request->kr_lat_degrees[$key],
                    'lat_minutes' => $request->kr_lat_minutes[$key],
                    'lat_second' => $request->kr_lat_second[$key],
                    'lat_direction' => $request->kr_lat_direction[$key],
            ...
            DB::commit();
            return redirect()->route('permohonan.pertimbangan-teknis.reklamasi.show', ['id' => $insert->id])->with(['success' => 'Data berhasil dibuat !']);
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    {
        $type = 'Pertek/Reklamasi';
        $bulanRomawi = $this->getRomawi(date('m'));
        $year_now = date('Y');

        $obj = DB::table('permohonan_pt_reklamasi')
